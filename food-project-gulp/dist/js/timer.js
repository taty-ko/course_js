...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const deadline = '2022-11-28'; // задем переменную в виде строки, потому что строка нормльно парсится.
    ...
    function getTimeRemaining(endTime) { // задача функции  - получить разницу между датами начала и конца.
        ...
        const days = Math.floor(t / (1000*60*60*24)); //Math.floor - округление до ближайшего целого.
                                                     // 1000*60*60*24 - 1000 миллисекунд в минуте, минут в часе, часов в сутках.

        const hours = Math.floor((t / (1000*60*60) % 24)); // % 24 - остаток от деления на 24, сутки.
                                                         // 1000 милиссекунд, 60 минуту, на 60 минут в часе.                                            
        const minutes = Math.floor((t / 1000 / 60) % 60); // делим на 1000 получаем кол-во секунд и делим  на 60 - получаем кол-во минут, % 60, чтобы получить минуты, не больше 60.

        const sec = Math.floor((t / 1000  % 60)); // / делим на 1000 получаем количество секунд в миллисекундах.
